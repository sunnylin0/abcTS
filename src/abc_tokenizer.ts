...
//	getKeyAccidental(str: string): { len: number; token: { acc: string; note: string } } | { len: number; warn: string };
...
//	getVoiceToken(line: string, start: number, end: number): { len: number; token: string } | { len: number; err: string };
...
//	getMeasurement(tokens: { type: string; token: string; start: number; end: number }[]): { used: number; value: number } | { used: number };
...
class AbcTokenizer {
	...
	getMode(str: string): { len?: number; token?: string } {
		...
		if (firstThree.length > 1 && (firstThree[1] === ' ' || firstThree[1] === '^' || firstThree[1] === '_' || firstThree[1] === '='))
			firstThree = firstThree[0]; // This will handle the case of 'm'
		switch (firstThree) {
	...
	getClef(str: string): { len?: number; token?: string; explicit?: boolean; warn?: string } {
		let strOrig = str;
		let i = this.skipWhiteSpace(str);
		if (this.finished(str, i))
			return { len: 0 };
	...
	// This is called a lot and there is no obvious tokenable items, so this is broken apart.
	...
	getMeat(line: string, start: number, end: number): { start: number; end: number } {
		...
		let comment = line.indexOf('%', start);
	...
	tokenize(line: string, start: number, end: number): { type?: string; token?: string; start?: number; end?: number; continueId?: boolean }[] {
		...
		let ret = this.getMeat(line, start, end);
		start = ret.start;
		end = ret.end;
		...
		while (start < end) {
			if (line[start] === '"') {
				...
				start = i + 1;
			} else if (this.isNumber(line[start])) {
				i = start + 1;
				while (i < end && this.isNumber(line[i])) i++;
				tokens.push({ type: 'number', token: line.substring(start, i), continueId: this.isLetter(line[i]), start: start, end: i });
				start = i + 1;
			} else if (line[start] === ' ') {
				i = start + 1;
			} else {
	...
	getFraction(line: string, index: number): { value: number; index: number } {
		...
		if (line[index] === '/') {
			...
			if (line[index] === '/') {
				...
				if (ret2.num === 0 && iSave === index)	// If we didn't use any characters, it is an implied 
					ret2.num = 2;
				if (ret2.num !== 0)
					den = ret2.num;
				index = ret2.index;
