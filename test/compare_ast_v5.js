...
testABCStrings.forEach((abc, idx) => {
	...
	// 2. 新版渲染 (NEW) - 將 Parse 與 Print 全數納入沙盒監控
			...
			get(target, prop) {
				...
				if (prop === 'path' && typeof origMethod === 'function') {
					return function (attr) {
						console.log(`[DEBUG Proxy NEW] 🎨 path() 繪製:`, typeof attr === 'object' ? (attr.path || attr) : attr);
				...
				if (prop === 'text' && typeof origMethod === 'function') {
					return function (x, y, textStr, attr) {
						console.log(`[DEBUG Proxy NEW] 📝 text() 繪製: "${textStr}" at (${x}, ${y})`);
						const res = origMethod.apply(target, arguments);
	...
	// 將 ABC 樂譜字串與必要類別掛載進沙盒
	newContext.__sandboxAbcString = String(abc);
	newContext.window.__sandboxAbcString = String(abc);
	newContext.__sandboxPaper = printerPaper;

	console.log(`[DEBUG NEW] 進入沙盒執行 Parse 與 PrintABC...`);

	// 💡 關鍵修復：在沙盒內部同時執行 Parse 與 Print，並吞掉 $break
	const vmResult = vm.runInContext(`
    (function() {
      try {
        var str = window.__sandboxAbcString || __sandboxAbcString;
        if (!str) throw new Error("沙盒內部的 __sandboxAbcString 為空！");

        var parser = new window.AbcParse();
        parser.parse(str);
        
        var printer = new window.ABCPrinter(window.__sandboxPaper);
        printer.printABC(parser.getTune());
        
        return { success: true };
      } catch (err) {
        // 🎯 嚴格判定：必須真的是 Prototype.js 的 $break 控制物件
        var isRealBreak = (err === window.$break) || (err && err.name === '$break');
        ...
        return {
          ...
          stack: err ? (err.stack || String(err)) : 'no stack'
        };
      }
    })()
  `, newContext);
	...
	if (!vmResult.success) {
		...
		console.error(`  Error Message:`, vmResult.message || vmResult.errStr);
		console.error(`  📍 Real Stack Trace:\n`, vmResult.stack);
		process.exit(1);
	}

	console.log(`[DEBUG NEW] 沙盒執行完畢！結果:`, vmResult);
	console.log(`[DEBUG NEW] 新版 (NEW) 收集到的 Log 筆數: ${newPaper.drawLog.length}`);
