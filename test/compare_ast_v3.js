...
testABCStrings.forEach((abc, idx) => {
    ...
    try {
        ...
        console.log(`[DEBUG 1] SvgClass 存在: ${!!SvgClass}`);
        ...
        if (SvgClass) {
            ...
            // 💡 印出 SvgClass 實例上到底有哪些繪圖方法
            console.log(`[DEBUG 2] realSvgInst 原型方法:`, Object.getOwnPropertyNames(Object.getPrototypeOf(realSvgInst)));

            printerPaper = new Proxy(realSvgInst, {
                get(target, prop, receiver) {
                    // 💡 印出新版在繪圖過程中存取了哪些屬性/方法
                    if (typeof prop === 'string' && !['svg', 'parentElement', 'currentGroup'].includes(prop)) {
                        console.log(`[DEBUG 3 Proxy Access] Calling method: ${prop}`);
                    }
                    ...
                    if (prop === 'path' && typeof origMethod === 'function') {
                        return function (attr) {
                            console.log(`[DEBUG 4 LOGGED] path() called!`, attr);
                    ...
                    if (prop === 'text' && typeof origMethod === 'function') {
                        return function (x, y, textStr, attr, targetEl) {
                            console.log(`[DEBUG 4 LOGGED] text() called: "${textStr}"`);
                            const res = origMethod.apply(target, arguments);
        ...
        console.log(`[DEBUG 5] 開始執行 newPrinter.printABC()...`);
        newPrinter.printABC(newParser.getTune());
        console.log(`[DEBUG 6] newPrinter.printABC() 順利執行結束！`);

    } catch (e) {
        console.log(`\n❌ [Stage D Exception Captured!]`);

        // 💡 1. 嘗試提取跨 VM 環境的 Stack Trace 行號
        const stackTrace = e?.stack || (new Error()).stack;

        // 💡 2. 解析物件的所有屬性名（防止 Serialized 物件印不出來）
        const propNames = e ? Object.getOwnPropertyNames(e) : [];
        console.log(`  Exception Type:`, Object.prototype.toString.call(e));
        console.log(`  Exception Properties:`, propNames);
        ...
        // 💡 3. 印出完整堆疊與錯誤行號
        console.log(`  📍 Call Stack / Line Numbers:`);
        console.log(stackTrace);

        // 判斷是否為 Prototype.js 中斷
