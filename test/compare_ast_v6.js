...
// 💡 正規化整個 Log 陣列，讓 OLD 與 NEW 的資料結構完全對齊
function normalizeDrawLog(drawLog) {
  return drawLog
    .filter(item => {
      if (!item) return false;
      // 💡 保留 setSize，只過濾 response 設定（若需要比對 setSize 的話）
    ...
    .map(item => {
      ...
      if (copy.type === 'path') {
        ...
        if (copy.attr && copy.attr.path) {
          copy.attr.path = normalizePathString(copy.attr.path);
        }
      }
      ...
      // 2. 處理 Text 座標浮點數
      if (copy.type === 'text') {
        copy.x = Number(Number(copy.x).toFixed(6));
        copy.y = Number(Number(copy.y).toFixed(6));
      }

      // 3. 💡 處理 setSize 浮點數微小誤差 (例如 805.0000000000001 -> 805)
      if (copy.type === 'setSize') {
        copy.w = Number(Number(copy.w).toFixed(6));
        copy.h = Number(Number(copy.h).toFixed(6));
      }

      // 4. 統一預設樣式
      if (copy.attr) {
        if (!copy.attr.stroke) copy.attr.stroke = 'none';
        if (!copy.attr.fill) copy.attr.fill = '#000000';
        if (copy.attr.fill === '#0') copy.attr.fill = '#000000';
      }
      return copy;
    });
}
...
function printDrawLogDiff(oldLog, newLog) {
  ...
  const formatItem = (x) => {
    ...
    if (x.type === 'path') {
      ...
      const fill = x.attr?.fill || '#000000';
      return `path: ${pStr.substring(0, 60)}${pStr.length > 60 ? '...' : ''} (stroke=${stroke}, fill=${fill})`;
    }
...
testABCStrings.forEach((abc, idx) => {
  ...
  try {
    deepCompare(normalizedOldLog, normalizedNewLog, `DrawLog[${idx}]`);
