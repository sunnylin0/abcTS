...
testABCStrings.forEach((abc, idx) => {
    ...
    try {
        ...
        // 嘗試傳給 NewABCPrinter (支援直接傳 MockPaper，或傳由 Svg 包裹的 MockPaper)
        ...
        if (SvgClass) {
            try {
                ...
                // 將指令改記錄到 newPaper.drawLog
                svgInst.path = (p) => newPaper.path(p);
                svgInst.text = (x, y, t, a) => newPaper.text(x, y, t, a);
                paperArg = svgInst;
            } catch (err) {
                paperArg = newPaper;
            }
        ...
        if (!isBreakOrEmpty) {
            console.error(`❌ New Printer Fatal Error:`, e);
            process.exit(1);
        }
    }
